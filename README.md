...
### The fire class accepts a url to the fire spritesheet asset.

```js
const fire = new Fire("./src/assets/pewpew.png");
```

### You can stop and start the fire instance animation by using the following methods in Application.js's init method:

```js
await fire.start(); // Starts the fire animation
fire.stop(); // Stops the fire animation
```

### You need to append the fire instance's container to a html DOM element:
